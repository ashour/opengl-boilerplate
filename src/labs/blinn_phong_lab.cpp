...
BlinnPhongLab::BlinnPhongLab(const Window& window) : Lab(window)
{
    _window.set_clear_color(SCENE_CLEAR_COLOR);

    _shader = std::make_shared<Shader>("resources/shaders/blinn_phong.vert",
...
void BlinnPhongLab::on_ui_render(UI& ui) { movement_help_ui(ui); }
